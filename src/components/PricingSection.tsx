...
const PricingSection = () => {
  const pricingOptions = [{
    ...
    features: ["Участие в полной программе мастермайнда", "Материалы и презентации всех спикеров", "Кофе-брейки и обед премиум-класса", "Доступ в чат поддержки на 30 дней", "Сертификат участника"],
    isPopular: false
  }, {
    name: "VIP",
    price: "75 000",
    originalPrice: "95 000",
    features: ["Все преимущества тарифа Стандарт", "Приоритетные места в первых рядах", "Индивидуальная 30-минутная консультация", "Доступ в чат поддержки на 90 дней", "Приглашение на закрытый ужин с экспертами", "Эксклюзивные бонусные материалы"],
    isPopular: true
  }];
  return <section className="py-24 px-6 bg-card/50">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {pricingOptions.map((option, index) => <div key={index} className={`card-premium p-8 relative group hover:scale-[1.02] transition-all duration-300 ${option.isPopular ? 'border-2 border-gold/30' : ''}`}>
              {option.isPopular && <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
              ...
              <div className="space-y-4 mb-8">
                {option.features.map((feature, featureIndex) => <div key={featureIndex} className="flex items-start gap-3">
              ...
              <Button variant={option.isPopular ? "gold" : "goldOutline"} className="w-full font-modern font-semibold text-lg py-6 h-auto">
